...
class TransNetV2Pipeline:
    ...
    def __init__(
        self, 
        model_path: str = "Sn4kehead/TransNetV2", 
        device: str = None
    ):
        """
        初始化 TransNetV2 推理管道。
    ...
    def get_video_frames(self, num_frames: int) -> tuple:
        """
        从当前视频位置读取指定数量的帧。
        
        该方法会同时返回原始尺寸帧和用于模型推理的缩放帧 (27x48)。
        ...
        Returns:
            tuple: 包含两个张量的元组:
                - frames (torch.Tensor): 原始尺寸帧，形状为 [1, N, H, W, C]
                - resized_frames (torch.Tensor): 缩放后的帧，形状为 [1, N, 27, 48, C]
                其中 N 为实际读取的帧数（可能小于 num_frames）
        
        Raises:
            ValueError: 当无法从视频读取任何帧时。
        
        Note:
            如果视频剩余帧数少于 num_frames，将返回所有剩余帧。
        """
        frames = []
        ...
        # 调整维度顺序: [N, C, H, W] -> [1, N, H, W, C]
        frames = frames.permute(0, 2, 3, 1).unsqueeze(0)
    ...
    def shot_detect(
        ...
        该方法会检测视频中的所有镜头边界，并将每个镜头保存为独立的视频文件。
        支持断点续传，如果中途中断可以从上次位置继续处理。
        
        Args:
            output_dir (str): 输出目录路径。将在该目录下创建:
                - shots_list.txt: 记录所有镜头的起止帧
                - shots/: 存放分割后的镜头视频文件
            segment_frames (int, optional): 每次处理的帧数。
                较大的值需要更多内存但处理更快。
                默认值: 1000
            threshold (float, optional): 镜头边界检测阈值，范围 [0, 1]。
                值越小越敏感（检测到更多镜头）。
                默认值: 0.2
            min_frames (int, optional): 最小镜头帧数。
                短于此值的镜头将被忽略。
                默认值: 120
        ...
        Raises:
            ValueError: 当未先调用 read_video() 加载视频时。
        ...
        Example:
            ...
            >>> 
            >>> # 基础使用
            >>> count = pipeline.shot_detect("./output")
            >>> 
            >>> # 高级配置
            >>> count = pipeline.shot_detect(
            ...     output_dir="./output",
            ...     threshold=0.15,      # 更敏感的检测
            ...     min_frames=60,       # 允许更短的镜头
            ...     target_width=1920,   # 裁剪到 1080p
            ...     target_height=1080
            ... )
        
        Note:
            - 输出文件命名格式: scene_XXX_SSSSSS_EEEEEE.mp4
              其中 XXX 为镜头序号，SSSSSS 为起始帧，EEEEEE 为结束帧
            - 支持断点续传：程序会自动读取 shots_list.txt 并跳过已处理部分
        """
        if self.reader is None:
            raise ValueError(
                "请先调用 `TransNetV2Pipeline.read_video()` 方法加载视频"
            )
        ...
        # 初始化计数器和状态变量
        scene_count = len(glob.glob(os.path.join(output_video_dir, "*.mp4")))
        ...
        while not video_read_out:
            try:
                next_segment, next_segment_resized = self.get_video_frames(segment_frames)
            except ValueError:
                break

            video_read_out = next_segment.shape[1] < segment_frames
            ...
            if len(scenes) == 1 and not video_read_out:
                prev_segment, prev_segment_resized = next_segment, next_segment_resized
                continue

            # 处理检测到的场景
            for scene in scenes[:-1] if len(scenes) > 1 else scenes:
                abs_start, abs_end = prev_frame_pos + scene[0], prev_frame_pos + scene[1]
                ...
                self._save_video(shot_frames, output_filename, abs_start, abs_end, video_fps)
            ...
            if scenes.size > 0:
                ...
                progress_bar.update(scenes[-1][0] if not video_read_out else scenes[-1][1])
    
        progress_bar.close()
        
        # 打印处理结果
        print(f"\n镜头检测完成！")
        print(f"成功保存: {scene_count} 个镜头")
